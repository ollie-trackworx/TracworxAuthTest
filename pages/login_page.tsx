...
import { AuthSocialProvider } from '../logic/auth_logic';
type LoginProps = {
  navigation: any;
};
...
const LoginPage: React.FC<LoginProps> = ({ navigation }) => {
  ...
  const { loading, error, login,loginWith } = useAuthcontext();
  ...
  const handleLoginWithGoogle = async () => {
    if(await loginWith(AuthSocialProvider.Google))
     navigation.navigate(Routes.initial)
  }
  return <KeyboardAvoidingView behavior="padding" style={styles.container}>
    <TouchableWithoutFeedback onPress={() => Keyboard.dismiss()}>
      ...
      >
        ...
        <Button onPress={handleLoginWithGoogle} icon="google" mode="text" style={{ ...styles.buttons, marginTop: 30 }} textColor="white" buttonColor={"#1D1D29"}> Sign in with Google </Button>
      </Column>

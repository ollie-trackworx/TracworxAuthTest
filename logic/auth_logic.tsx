...
GoogleSignin.configure({
  ...
  webClientId: '1064352884074-vc3s01cdsccrs59jiren4prsfgh218qu.apps.googleusercontent.com',
});
...
class FireAuthHandler implements AuthHandler {
    ...
    try {
      switch (service) {
        case AuthSocialProvider.Google:
          const googleCredential = FirebaseAuth.GoogleAuthProvider.credential(await signInWithGoogle());
          let user = (await FirebaseAuth().signInWithCredential(googleCredential))?.user;
          if (user) {
            return {
              uid: user.uid,
  ...
  onAuthStateChanged(listener: (user: User) => {}): any {
    FirebaseAuth().onAuthStateChanged((auth) => listener({
      uid: auth?.uid,
  ...
  async login(username: string, password: string): Promise<User | null> {
    try {
      ...
      let user = (await FirebaseAuth().signInWithEmailAndPassword(username, password)).user;
      return {
        uid: user.uid,
        authStatus: AuthStatus.LoggedIn,
      ...
      let message = `${e}`;
      if (message.includes("]"))
        throw new Error(message.split("]")[1].trim())
      throw e;
    }
